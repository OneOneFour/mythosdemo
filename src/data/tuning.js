/* LAYER data — TUNABLES: the frozen DESIGN. Base values only. Never written.
   ...
   IMPORT RULE, ENFORCED: only `model/mods.js` may import this file, and
   ...
   a real substance or machine id, fails the content lint. */
...
export const TUNABLES = [
  ...
  /* A SIXTH OF WALK, NOT HALF. Half walk measured as no tax at all -- the whole
     world climbed in 7.5 s against a 480 s clock at every load a player
     carried. At 10 the same climb costs 22.4 s and a carrier chain can beat
     it. This makes up expensive for the PLAYER; `segUp` is cargo's half. */
  ...
  /* SEGMENT TRANSPORT. Eight rows pricing one rule: a carrier rises only while
     something is actively turning it, and slides back down under its own
     weight for nothing. `rules/drive.js` is the only reader, and its motion
     expression is `need = segBase + segLoad * mass * slope` against the
     drivetrain's supply. Weighted descent is what that ALREADY produces at
     zero supply, so there is no separate descend number and no charge gate.

     `segUp` AND `segDown` ARE THE SAME NUMBER: nothing on a cable rises
     faster than it falls. `segLoad` PUTS ONE CRANK'S STALL EXACTLY ON THE
     BURDEN CAP, and that stall must stay REACHABLE. */
  ...
  /* `kiln_divine` is twice as fast because of this one line and nothing
     else. */
  ...
  /* encumbrance, in TALENTS. `burden` is the hard cap, `burdenSoft` the
     fraction of it where climb speed begins to fall off, `burdenClimbFloor`
     the climb multiplier AT the cap -- the tick before ladder-up and hop are
     refused outright. Walking on level ground and every downward movement are
     never scaled by any of the three.

     `burdenSoft` IS `riderMass / burden`, so the falloff starts the moment
     your pockets weigh as much as your own body (8 T of 40). The curve is
     linear from 1.0 at the knee to the floor at the cap, so it stays shallow
     near the knee and only bites past half a cap. */
  ...
  /* the draft. `offerSize` is how many of a tier's still-undrafted rows a god
     lays out, and a tier with fewer left offers fewer because
     `rules/draft.js` never pads. `rerollCost` is in FAVOUR, spent with the
     god whose trial raised the offer, and 2 against the running totals buys
  ...
  /* the altar's arrival. The director withholds cycle 1's altar until the
     player has climbed back out of their own shaft, and this is the deadline
     on that wait -- the earliest instant the beat sheet allows, so a player
     who never digs meets it there rather than at a time this row invented. A
     player who does dig never reaches it. Measured against `run.t`, simulated
     seconds at the fixed substep. */
  ...
  /* inventory. `invSlots` is `run.mainSlots` at reset; `quickbarSlots` is the
     tail of `run.inv` past it. A slot count is content, read through
     `eff()`. */
  ...
  /* deposit depletion: scales a `deposit` substance's `tile.charge`, how many
     units one native tile yields before it is gone. It does NOT touch `hard`,
     so it changes the SIZE of a find and never the rate of a swing. Scoped
     like `hard` and `toolTier`, rounded and floored at 1 by both readers, so
     no bend can make a tile yield nothing. */
  ...
  /* yield quality: not every unit a tile WOULD give actually drops. Base 1.0,
     so a real ore is unaffected and every unit lands; `soil` and `stone` are
     overridden low, because they are filler you tunnel through rather than a
     vein of anything.

     Rolled once per unit ORE INCLUDED -- a roll against 1.0 always passes --
     so the position of every downstream `rand()` draw does not depend on
     which substance is being mined. */
  ...
  /* tree regrowth. `log` is the only fuel a player can mine, so a felled forest
     is a run that has quietly ended. `treeGrowSecs` IS 180 AND NOT 90 -- 90
     is `brandSecs`, this game's unit of one long errand, and a tree ought to
     cost more. 180 s is between a third and a half of every shipped deadline,
     so a seed planted in a trial's first third pays back inside it.

     `seedYield` IS 2 BECAUSE 2 IS THE SMALLEST INTEGER THAT COMPOUNDS, and it
     stays a VALUE rather than a chance, because regrowth that sometimes gives
     nothing sometimes ends the timber economy. RAISING IT CHANGES THE `rand()`
     STREAM: two draws per seed. */
